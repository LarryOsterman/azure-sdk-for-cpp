...
namespace Azure { namespace Security { namespace Attestation { namespace _detail {
  ...
  std::unique_ptr<Cryptography::AsymmetricKey> BCryptAsymmetricKey::ImportPublicKey(
      std::string const& publicKeyToImport)
  {
    ...
    if (strcmp(publicKeyInfo.get()->Algorithm.pszObjId, szOID_ECC_PUBLIC_KEY) == 0)
    {
      ...
      THROW_IF_WIN32_BOOL_FALSE(CryptImportKey(
          ...
          CRYPT_EXPORTABLE,
          key.addressof()));
      return std::unique_ptr<Cryptography::AsymmetricKey>(
   ...
   *
   */
  std::unique_ptr<Cryptography::AsymmetricKey> BCryptAsymmetricKey::ImportPrivateKey(
  ...
  BCryptException::BCryptException(std::string const& what) : runtime_error(GetBCryptError(what)) {}
