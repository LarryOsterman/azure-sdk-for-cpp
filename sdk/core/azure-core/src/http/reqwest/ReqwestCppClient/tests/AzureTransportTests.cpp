...
namespace Azure { namespace Core { namespace Test {
  ...
  //    auto request = Azure::Core::Http::Request(Azure::Core::Http::HttpMethod::Put, host,
  //    &stream);

  //    // Request will be cancelled from main thread throwing the exception
  //    EXPECT_THROW(m_pipeline->Send(request, cancelThis),
...
#error "Unknown platform"
  ...
  void LogRawBuffer(std::ostream& os, const uint8_t* buffer, size_t size)
  {
      ...
      std::string expectedBody)
  {
    ...
    // if (!expectedBody.empty())
    //{
    //   GTEST_LOG_(INFO) << "Expected body: ";
    //   LogRawBuffer(
    //       GTEST_LOG_(INFO),
    //       reinterpret_cast<const uint8_t*>(expectedBody.data()),
    //       expectedBody.size());
    // }
    //    GTEST_LOG_(INFO) << "Actual body:";
    //    LogRawBuffer(GTEST_LOG_(INFO), bodyVector.data(), bodyVector.size());
    EXPECT_EQ(bodySize, size);
