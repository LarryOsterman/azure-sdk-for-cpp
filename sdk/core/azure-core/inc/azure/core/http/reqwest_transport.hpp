...
namespace Azure { namespace Core { namespace Http { namespace RustTransport {
  namespace _detail {
    ...
    namespace RustInterop {
      // The CppStream is a declared but undefined C++ type that is used to pass the C++ stream to
      // rust. Implement this type in the C++ code to pass the stream to rust, enabling cancellation
      // support.
  ...
  class RustHttpTransport : public Azure::Core::Http::HttpTransport {
  public:
    RustHttpTransport() : m_rustReqwestWrapper{_detail::RustInterop::reqwest_wrapper_new()} {}
    virtual ~RustHttpTransport() {}
    ...
    {
      _detail::AutoRuntimeContext runtimeContext{_detail::RustInterop::runtime_context_new()};

      RustHttpRequest rustRequest{request, context};
      _detail::AutoRustHttpResult rustHttpResult{_detail::RustInterop::reqwest_wrapper_send(
          runtimeContext.get(), m_rustReqwestWrapper.get(), rustRequest.GetRustRequest())};

      if (result_has_error(rustHttpResult.get()))
      {
          // Extract the error from the result and return it. This is a destructive operation, so release the pointer.
        ...
        throw Azure::Core::Http::TransportException(
            "Rust API Call failed: " + std::string(message));
      }
  ...
  private:
    _detail::AutoReqwestWrapper m_rustReqwestWrapper;

    std::unique_ptr<RawResponse> RawResponseFromRustResponse(
        _detail::AutoRuntimeContext&& runtimeContext,
        RustHttpResponse&& rustResponse)
    {
